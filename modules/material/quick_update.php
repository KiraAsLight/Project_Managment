...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $query .= " WHERE material_id = ? AND division = 'Purchasing'";

        $stmt = $conn->prepare($query);
        $stmt->bind_param("sdis", $status, $progress_percent, $_SESSION['user_id'], $material_id);
        ...
        if ($stmt->execute()) {
            ...
            log_activity(
                $conn,
                $_SESSION['user_id'],
                'Quick Update Procurement',
                "Quick update procurement for '{$material['name']}' to {$status} (Material ID: {$material_id})"
            );
